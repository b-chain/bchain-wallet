...
int createBchaind()
{
	...
	std::string exe_directory = base_directory + "\\bchaind.exe";
	size_t size = exe_directory.length();
	wchar_t* buffer = new wchar_t[size + 1];
	MultiByteToWideChar(CP_ACP, 0, exe_directory.c_str(), size, buffer, size * sizeof(wchar_t));
	buffer[size] = 0;
...
int APIENTRY wWinMain(HINSTANCE instance, HINSTANCE prev, wchar_t *command_line,
  ...
  TerminateProcess(stProcessInfo.hProcess, 300);
